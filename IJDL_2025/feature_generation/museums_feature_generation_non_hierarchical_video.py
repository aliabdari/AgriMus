...
if __name__ == '__main__':
    ...
    if base_features == base_features_list[0]:
        frame_video_representation = [('Mean'), ('Max'), ('Median')]

    for fv in tqdm(frame_video_representation):
        path_tensors = f'{base_features}/videos'
        path_output = f'{base_features}/museums_{fv}'
        ...
        for idx_m, m in enumerate(museums):
            ...
            for idx_r, r in enumerate(m['rooms']):
                ...
                for idx_v, v in enumerate(m['rooms'][r]):
                    ...
                    features_museum.append(features)
                # features_museum.extend(features_rooms)
            print(path_output)
            torch.save(aggregation_func(fv, torch.stack(features_museum)), path_output + os.sep + 'museum_' + str(idx_m) + '.pt')
