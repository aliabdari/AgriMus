...
def provide_features(base_features, fvr):
    if len(base_features) == 1:
        # Load Queries
        path_queries = f'../feature_generation/{base_features[0]}/videos/'
        # queries_text = [q.split('.')[0] for q in os.listdir(path_queries)]
        queries = {q.split('.')[0] : torch.load(path_queries + q, weights_only=True).cpu() for q in os.listdir(path_queries)}
